...
class SupervisorApp:
    ...
    def load_dashboard_image(self):
        # Load the image
        image_path = "C:/Users/junsheng/Desktop/Freelance/project2-main/build/assets/frame0/default-monochrome1.png"
    ...
    def setup_data_entry_widgets(self):
        ...
        self.date = DateEntry(self.data_entry_frame, date=datetime.now().date(), font=('Gill Sans MT', 13))
    ...
    def setup_tasks_entry_widgets(self):
        ...
        self.task_entry.place(x=350, y=30)
        ...
        self.worker_entry.place(x=350, y=80)


        self.due_date_label = ttk.Label(self.task_assignment_frame, text='Due Date (YYYY-MM-DD):', font=('Gill Sans MT', 13))
        self.due_date_label.place(x=130, y=130)
        self.due_date_entry = ttk.Entry(self.task_assignment_frame, font=('Gill Sans MT', 13), width=20)
        self.due_date_entry.place(x=350, y=130)


        self.assign_button = ttk.Button(self.task_assignment_frame, text='Assign', command=self.assign_task, style='Bold.TButton')
        self.assign_button.place(x=600, y=80, width=160, height=50)
    ...
    def setup_tasks_table(self):
        self.task_table = ttk.Treeview(self.table_frame3, selectmode=BROWSE,
                                       columns=('ID', 'Task', 'Assigned To', 'Status', 'Due Date'))
        ...
        self.task_table.heading('Due Date', text='Due Date', anchor=CENTER)

        self.task_table.column('#0', width=0, stretch=NO)
        self.task_table.column('#1', width=50, stretch=NO)
        self.task_table.column('#2', width=200, stretch=NO)
        self.task_table.column('#3', width=100, stretch=NO)
        self.task_table.column('#4', width=100, stretch=NO)
    ...
    def setup_tasks2_table(self):
        ...
        self.task2_table.heading('Due Date', text='Due Date', anchor=CENTER)
        ...
        self.task2_table.column('#3', width=100, stretch=NO)
    ...
    def load_tasks(self):
        ...
        conn = sqlite3.connect('users.db')
        cursor = conn.cursor()
        cursor.execute('SELECT id, task, assigned_to, status, due_date FROM tasks')
        tasks = cursor.fetchall()
        conn.close()

        for task in tasks:
            self.task_table.insert("", "end", values=(task[0], task[1], task[2], task[3], task[4]), iid=task[0])
    ...
    def load_tasks2(self):
        ...
        conn = sqlite3.connect('users.db')
        cursor = conn.cursor()
        cursor.execute("SELECT id, task, status, due_date FROM tasks WHERE assigned_to=?", (self.username,))
        for task in cursor.fetchall():
            self.task2_table.insert("", "end", values=(task[0], task[1], task[2], task[3]), iid=task[0])
        conn.close()
    ...
    def assign_task(self):
        ...
        if not self.is_valid_date(due_date):
            mb.showerror("Invalid Date", "Please enter a valid date in the format YYYY-MM-DD")
            return

        conn = sqlite3.connect('users.db')
        cursor = conn.cursor()
        cursor.execute("INSERT INTO tasks (task, assigned_to, status, due_date) VALUES (?, ?, ?, ?)", (task, worker, "Incomplete", due_date))
        conn.commit()
        conn.close()
        self.load_tasks()
